...
本文是对epoll的实战，借助epoll的ET非阻塞IO工作模式，实现单线程的简易服务器，主要功能是接收客户端数据并回发。

下面分别是客户端、服务器代码，最后通过shell脚本进行压力测试，最后是实验截图。

# 2 客户端代码

客户端是接收控制台输入，并将输入结果发送到服务器端，接收服务器端回发！
