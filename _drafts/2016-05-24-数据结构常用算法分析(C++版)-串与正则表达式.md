...
字符串是我们编程最常面对的类型之一，本节就简单来看看字符串的存储与使用，
本节从C和C++两个方面分别看一下字符串的定义、存储与使用，
...
Regex作为字符串匹配、处理的利器,在这一节怎么能少了这以法宝呢？
